...
export const createPaymentIntentWithPaymentMethod = async (
  ...
  // The list all Customers endpoint can return multiple customers
...
export const payWithoutWebhooks = async (
  req: Request,
  res: Response,
): Promise<Response<any>> => {
  const {
...
export const chargeCardOffSession = async (
  req: Request,
  res: Response,
): Promise<Response<any>> => {
  let paymentIntent;
...
export const paymentSheetSubscription = async (req: Request, res: Response) => {
  const { secret_key } = getKeys();
  ...
  if (typeof subscription.pending_setup_intent === 'string') {
    ...
    throw new Error(
      `Expected response type string, but received: ${typeof subscription.pending_setup_intent}`,
    );
  }
};

export const ephemeralKey = async (
  req: Request,
  res: Response,
): Promise<Response<any>> => {
  const { secret_key } = getKeys();
...
export const issuingCardDetails = async (
  req: Request,
  res: Response,
): Promise<Response<any>> => {
  const { secret_key } = getKeys();
...
export const financialConnectionsSheet = async (
  req: Request,
  res: Response,
): Promise<Response<any>> => {
  const { secret_key } = getKeys();
